...
Phone-first lighting control. Bookmark `/signalk-n2k-displays/` when glass is unreadable. PUT goes to v1 paths (`electrical.displays.*`), not plugin REST. Chrome follows the phone light/dark setting.

![Display lighting webapp on a phone in dark mode, Auto and Night selected, intent 40%, Navico 4/10 red palette and Raymarine 40% Red/Black](docs/webapp.png)
...
**In:** Time / sun / lux steering (cascade lux → sun → time), intent paths, maps, group sync, power-on resync, PUT on v1 paths, embeddable webapp.
